...
cond_num = 1e4;     % condition number
sparsity = 0.7;     % more sparse -> harder signal recovery
noise_level = 0.5;  % higher noise
lambda_frac = 1e-2; % much smaller regularization (less sparsity)

% Step 1: Generate ill-conditioned, correlated matrix A
[U, ~] = qr(randn(m,m),0);   % m x m orthonormal
[V, ~] = qr(randn(n,n),0);   % n x n orthonormal

k = min(m,n);
% Stronger singular value decay
s = logspace(0, -log10(cond_num), k);
S = diag(s);
...
% Add correlations among columns to make optimization landscape trickier
A(:,2:end) = A(:,2:end) + 0.02*A(:,1);

% Generate sparse true solution x_true
x_true = sprandn(n,1,sparsity);

% Generate noisy observations
b = A*x_true + noise_level*randn(m,1);

% Set Ridge regularization parameter
lambda_max = norm(A)^2;
lambda = lambda_frac * lambda_max;

% Normalize columns to avoid numerical explosion
A = A ./ vecnorm(A);

% Optional: display info
disp(['Condition number of A: ', num2str(cond(A))]);
disp(['Lambda used: ', num2str(lambda)]);
disp(['Number of nonzeros in x_true: ', num2str(nnz(x_true))]);

%%%%%%%%%
x0 = zeros(n,1);
gamma = 1 / ((norm(A)^2) + lambda);
delta = 1e-4;
maxit = 1000;
tol = 1e-10;
...
fprintf('Running PGA...\n');
[x_pga, info_pga] = pg_nrr(A, b, lambda, x0, gamma, maxit, tol);

%=======================

% Run Accelerated Proximal Gradient (FISTA)
fprintf('Running FISTA...\n')
[x_fis, info_fis] = fista_nrr(A, b, lambda, x0, gamma, maxit, tol);

%=========================

% Run AA-PG (Anderson Acceleration Proximal Gradient Algorithm with guard)
fprintf('Running AA-PG...\n');
% m = 10
fprintf('Running m = 10...\n');
[x_aa10, info_aa10, acc_aa10] = aa_pg_nrr(A, b, lambda, x0, gamma, 10, maxit, tol);
% m = 20
fprintf('Running m = 20...\n');
[x_aa20, info_aa20, acc_aa20] = aa_pg_nrr(A, b, lambda, x0, gamma, 20, maxit, tol);

%===========================

% Run CROP-PG (Guarded CROP Proximal Gradient Algorithm)
fprintf('Running CROP-PG...\n');
% m = 10
fprintf('Running m = 10...\n');
[x_crop10, info_crop10, acc_crop10] = crop_pg_nrr(A, b, lambda, x0, gamma, delta, 10, maxit, tol);
% m = 20
fprintf('Running m = 20...\n');
[x_crop20, info_crop20, acc_crop20] = crop_pg_nrr(A, b, lambda, x0, gamma, delta, 20, maxit, tol);
...
% m = 10
fprintf('Running m = 10...\n');
[x_crop_exact10, info_crop_exact10, acc_crop_exact10] = crop_pg_nrr_exact(A, b, lambda, x0, gamma, delta, 10, maxit, tol);
% m = 20
fprintf('Running m = 10...\n');
[x_crop_exact20, info_crop_exact20, acc_crop_exact20] = crop_pg_nrr_exact(A, b, lambda, x0, gamma, delta, 20, maxit, tol);
